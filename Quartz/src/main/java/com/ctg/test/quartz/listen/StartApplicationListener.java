...
public class    StartApplicationListener implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void initMyJob()  {
        ...
        try {
            ...
            //定义一个TriggerKey
            TriggerKey triggerKey = TriggerKey.triggerKey("trigger3", TRIGGER_GROUP_NAME);
            CronTrigger trigger = (CronTrigger) scheduler.getTrigger(triggerKey);
            if (null == trigger) {
                Class clazz = MyQuartzJobBean.class;
                //定义一个JobDetail,其中的定义Job类，是真正的执行逻辑所在
                JobDetail jobDetail = JobBuilder.newJob(clazz).withIdentity("job3", JOB_GROUP_NAME).build();
                CronScheduleBuilder scheduleBuilder = CronScheduleBuilder.cronSchedule("0/30 * * * * ?");
                //定义一个Trigger
                trigger = TriggerBuilder.newTrigger().withIdentity("trigger3", TRIGGER_GROUP_NAME)
